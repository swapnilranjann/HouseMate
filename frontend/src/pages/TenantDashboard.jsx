...
import { Plus, Home, MapPin, Eye, Upload, Check, X, ShieldCheck, LayoutGrid, Clock, Calendar, MessageSquare, AlertCircle, Briefcase, ShoppingBag, Building, ArrowUpRight, PlusCircle, Globe, Layout, ChevronRight, Activity } from 'lucide-react';
...
const TenantDashboard = () => {
  ...
  const handleUpdateAppointment = async (id, status) => {
      try {
          ...
          fetchData();
          toast.success(`Protocol ${status.toUpperCase()}! Handshake finalized.`);
      } catch (err) {
  ...
  const handleCreateProperty = async (e) => {
    ...
    const loadingToast = toast.loading('Synchronizing asset with marketplace...');
    try {
      ...
      setImages([]);
      fetchData();
      toast.success('Asset live! Verified location data synchronized.', { id: loadingToast });
    } catch (err) {
      toast.error('Listing protocol rejected.', { id: loadingToast });
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <DashboardHeader 
        title={<>Owner <span className="text-sky-400">Command Center</span></>}
        subtitle={`Managing your high-yield portfolio. Verification HM_V_${JSON.parse(localStorage.getItem('house_user'))?.id?.slice(-5).toUpperCase()} active.`}
        icon={ShieldCheck}
        roleLabel="Market Intelligence"
        accentColor="sky"
        stats={[
          { label: "Active Listings", value: properties.length },
          { label: "Pending Protcols", value: appointments.filter(a => a.status === 'pending').length, highlight: true }
        ]}
      />

      <div className="flex flex-col lg:flex-row justify-between items-center gap-10 mb-16">
          <div className="bg-white p-2 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-2 overflow-x-auto custom-scrollbar">
              {['listings', 'appointments', 'metrics'].map(tab => (
                  <button 
                    key={tab}
          ...
          >
            {showForm ? <><X size={18} /> ABORT LISTING</> : <><PlusCircle size={18} /> CREATE NEW ASSET</>}
          </button>
      </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="mb-20">
                <div className="bg-white rounded-[4rem] border border-slate-100 shadow-2xl overflow-hidden relative">
                    <div className="bg-slate-50/50 p-12 md:p-16 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-10">
                        <div>
                            <h2 className="text-4xl font-serif italic font-black text-slate-900 leading-none mb-3 italic tracking-tighter uppercase italic">Boutique Asset Registration</h2>
                            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest flex items-center gap-2"><Globe size={14} className="text-sky-400" /> Mandatory Location Intelligence Required</p>
                        </div>
                    ...
                    <form onSubmit={handleCreateProperty} className="p-12 md:p-16 grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16">
                         <div className="space-y-10">
                            <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em] border-b border-slate-100 pb-4">01. Identity & Type</h4>
                            <div className="space-y-6">
                                <input type="text" placeholder="Editorial Name (e.g. Skyline Villa)" required className="w-full px-8 py-5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-8 focus:ring-sky-500/5 focus:border-sky-500 text-xs font-black tracking-widest transition-all outline-none" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                         ...
                         <div className="space-y-10">
                            <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em] border-b border-slate-100 pb-4">02. Spatial Intelligence</h4>
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <input type="text" placeholder="Plot / No." required className="w-full px-8 py-5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-sky-500/10 text-xs font-black tracking-widest transition-all outline-none" value={formData.number} onChange={e => setFormData({...formData, number: e.target.value})} />
                                </div>
                                <div className="relative">
                                    <MapPin className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300" size={20} />
                                    <input type="text" placeholder="Global Address Frequency" required className="w-full pl-16 pr-8 py-5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-sky-500/10 text-xs font-black tracking-widest transition-all outline-none" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                                </div>
                                <div className="relative">
                                    <Globe className="absolute left-6 top-1/2 -translate-y-1/2 text-sky-400" size={20} />
                                    <input type="url" placeholder="Google Maps Precision Link" required className="w-full pl-16 pr-8 py-5 bg-sky-50 border border-sky-100 rounded-2xl focus:ring-8 focus:ring-sky-500/10 text-xs font-black tracking-widest transition-all outline-none" value={formData.locationLink} onChange={e => setFormData({...formData, locationLink: e.target.value})} />
                                </div>
                            </div>
                         </div>
                         <div className="space-y-10">
                            <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em] border-b border-slate-100 pb-4">03. Assets & Media</h4>
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="border-2 border-dashed border-slate-100 rounded-[2.5rem] p-8 hover:border-sky-500 transition-all bg-slate-50/50 flex flex-col items-center gap-4 group cursor-pointer relative shadow-inner">
                                    ...
                                    <label htmlFor="dash-imgs-pro" className="cursor-pointer text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] underline underline-offset-8">TRANSMIT Visual Assets</label>
                         ...
                         <div className="md:col-span-3 pt-6">
                            <button type="submit" className="w-full py-6 bg-slate-900 border border-slate-900 hover:bg-black text-white rounded-3xl font-black tracking-[0.4em] text-[12px] shadow-2xl transition-all hover:-translate-y-1 active:scale-95">AUTHENTICATE & PUBLISH LISTING</button>
                         </div>
                    </form>
                </div>
            </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
        <AnimatePresence mode="wait">
                ...
                <motion.div key="listings" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                    {properties.map((p, idx) => (
                        <motion.div 
                            ...
                            className="bg-white rounded-[3.5rem] border border-slate-100 overflow-hidden group hover:shadow-2xl transition-all duration-700 hover:border-sky-100 flex flex-col relative"
                        >
                            <div className="relative h-64 overflow-hidden">
                                ...
                                <div className="absolute top-8 left-8 flex flex-col gap-3">
                                    <span className="bg-white/95 backdrop-blur-md px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.2em] text-slate-900 shadow-xl flex gap-2 items-center leading-none">
                            ...
                            <div className="p-10 grow flex flex-col">
                                <h3 className="text-3xl font-serif italic text-slate-900 group-hover:text-sky-600 transition-colors mb-2 truncate italic font-black uppercase italic tracking-tighter">{p.name}</h3>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-10">
                                    ...
                                    <div className="p-5 bg-slate-50 rounded-3xl flex flex-col items-center group-hover:bg-sky-50 transition-colors">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Portfolio Status</span>
                                        <span className={`text-[10px] font-black uppercase tracking-widest leading-none ${p.status === 'open' ? 'text-emerald-500' : 'text-rose-500'}`}>{p.status}</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-3 mt-auto">
                                    <button onClick={() => updatePropertyStatus(p._id, p.status === 'open' ? 'booked' : 'open')} className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-xl hover:-translate-y-1 ${p.status === 'open' ? 'bg-slate-900 text-white hover:bg-black' : 'bg-white text-slate-900 border border-slate-200'}`}>
                                        {p.status === 'open' ? 'SECURE ASSET' : 'OPEN FOR MARKET'}
                                    </button>
                    ...
                    {properties.length === 0 && (
                        <div className="col-span-full">
                            <EmptyState 
                                title="Portfolio Idle"
                                message="No premium assets currently logged in your command center. Initiate your first listing protocol to begin attracting verified inquiries."
                                icon={Layout}
                                actionText="START LISTING PROTOCOL"
                    ...
                    {appointments.length > 0 ? appointments.map((a, idx) => {
                        ...
                        return (
                            <motion.div 
                                ...
                                transition={{ delay: idx * 0.1 }}
                                className="bg-white p-12 rounded-[4rem] border border-slate-100 flex flex-col lg:flex-row items-center justify-between gap-12 group hover:shadow-2xl transition-all duration-700 hover:border-sky-300"
                            >
                                <div className="flex items-center gap-10 grow w-full">
                                    <div className="w-24 h-24 bg-slate-900 rounded-[2rem] flex items-center justify-center font-black text-4xl text-white shadow-2xl relative">
                                        {a.customerId?.name?.[0]}
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-4">
                                            <h4 className="text-3xl font-serif italic font-black text-slate-900 tracking-tight uppercase italic">{a.customerId?.name}</h4>
                                            <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest leading-none ${a.status === 'pending' ? 'bg-amber-50 text-amber-500' : a.status === 'approved' ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'}`}>
                                                Status: {a.status}
                                            </span>
                                        </div>
                                        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 text-slate-400">
                                            ...
                                            <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest"><Calendar size={14} className="text-indigo-400" /> Requested {new Date(a.createdAt).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-6 w-full lg:w-auto shrink-0">
                                    {a.status === 'pending' ? (
                                        <div className="flex gap-4 w-full lg:w-auto">
                                            <button onClick={() => handleUpdateAppointment(a._id, 'approved')} className="flex-1 lg:flex-none bg-emerald-500 hover:bg-black text-white px-10 py-5 rounded-2xl shadow-xl transition-all font-black text-[10px] uppercase tracking-widest">APPROVE PROTOCOL</button>
                                        ...
                                        <div className="flex items-center gap-4 w-full lg:w-auto">
                                            {existingChat ? (
                                                <Link to={`/chats/${existingChat._id}`} className="flex-1 lg:flex-none bg-slate-900 hover:bg-black text-white px-10 py-5 rounded-2xl flex items-center justify-center gap-3 text-[11px] font-black uppercase tracking-widest shadow-2xl transition-all">
                                                   OPEN TRANSMISSION <MessageSquare size={18} />
                                                </Link>
                                            ) : (
                                                <div className="px-10 py-5 bg-slate-50 border border-slate-100 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center">LINK PENDING</div>
                                            )}
                                        </div>
                                    )}
                                </div>
                        ...
                        <EmptyState 
                            title="Schedule Idle"
                            message="No pending handshake requests found. Your enterprise protocol is currently optimized for capacity."
                            icon={Calendar}
                            color="sky"
                        />
                    )}
            ...
            {activeTab === 'metrics' && (
                <motion.div key="metrics" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-20 text-center glass rounded-[4rem] border border-slate-100">
                    <Activity size={80} className="mx-auto text-sky-500 mb-8 animate-pulse" />
                    <h3 className="text-3xl font-serif italic font-black text-slate-900 mb-4 italic uppercase">Analytics Intelligence</h3>
                    <p className="text-slate-400 text-sm max-w-md mx-auto leading-relaxed">Advanced yield metrics and neighborhood conversion analytics are being synchronized. Check back shortly for deep-market insights.</p>
                </motion.div>
            )}
        </AnimatePresence>
      </motion.div>
